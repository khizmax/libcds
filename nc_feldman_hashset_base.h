#ifndef CDSLIB_INTRUSIVE_DETAILS_NC_FELDMAN_HASHSET_BASE_H
#define CDSLIB_INTRUSIVE_DETAILS_NC_FELDMAN_HASHSET_BASE_H
...
/*
    Class similar to multilevel_array. This class is an inner class for NCFeldmanHashSet (no collision FeldmanHashSet).
    The only difference with multilevel_array is that this class contains lists inside instead of values (as multilevel_array does),
    also some new traits were added: list_type, list_allocator, value_equal.     
*/
namespace cds { namespace intrusive {

    /// NCFeldmanHashSet related definitions
    ...
    */
    namespace feldman_hashset {
        ...
        struct traits
        {
            /// Mandatory type.
            /**
                This type must also contain traits for list.
                Theoretically, any lock-free intrusive iterable list is supported.
                List must also support empty(), insert(), erase() methods. 
                For example, for cds lists you can provide item_counter.  
                ...
                typedef cds::intrusive::IterableList<gc_type, int_item, traits> list; //list type to use in NCFeldmanHashSet       

                \endcode
            */
            typedef cds::opt::none list_type;
   
            /// List allocator
            /**
                Allocator for inner lists. The allocator is used for creating inner lists when insertion to the empty node happens. 
            ...
            */
            ...
            /// Mandatory functor that checks values equality.
            /**
                Required for searching the right element inside same-hashed list (used in contain(), find(), erase() methods).
            */
            typedef cds::opt::none equal;
        ...
        /**
            ...
            - \p feldman_hashset::list_type - mandatory option, type of list to use inside.
                @copydetails traits::list_type
            - \p feldman_hashset::list_allocator - list allocator.
                @copydetails traits::list_allocator
            ...
            - \p feldman_hashset::value_equal - mandatory option, checks values equality.
                @copydetails traits::value_equal
...
#   endif
        ...
        public:
            ...
            //typedef LazyList<cds::gc::HP, value_type> list_type;
            typedef typename Traits::node_allocator node_allocator;
            typedef typename Traits::list_allocator list_allocator;
            ...
            static_assert(!std::is_same< list_type, cds::opt::none >::value, "list_type must be specified");
            ...
            struct list_hash_accessor {
                hash_type operator()(const list_type& l) {
                    return hash_accessor()(*l.begin());
                }    
            };
            
        protected:

            typedef cds::details::marked_ptr< list_type, 3 > node_ptr;  ///< pointer to list instead of value 
        ...
        public:
            //changed
            multilevel_array(size_t head_bits = 8, size_t array_bits = 4 )
        ...
        protected:
            ...
            union converter {   // also converts to list instead of values
                list_type * pData;
                array_node * pArr;

                converter(list_type * p)
                    : pData(p)
                {}
            ...
            static array_node * to_array(list_type * p)
            {
                return converter(p).pArr;
            }
            static list_type * to_node(array_node * p)
            {
        ...
        private:
            bool expand_slot(array_node * pParent, size_t idxParent, node_ptr current, size_t nOffset)
            {
                ...
                typename hash_splitter::uint_type idx = hash_splitter( list_hash_accessor()(*current.ptr()), nOffset ).cut( 
...
#endif // #ifndef CDSLIB_INTRUSIVE_DETAILS_NC_FELDMAN_HASHSET_BASE_H

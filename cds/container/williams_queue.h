...
#define BITS_PTR 48
#define BITS_COUNTER 16
#define BITS_INTERNAL_COUNT 62
            const static uintptr_t MASK = ~((uintptr_t)1 << BITS_PTR);

            struct counted_node_ptr
            {
                uintptr_t ptr_and_external_count;

                int get_external_count(){
                    return (int)((uintptr_t)ptr_and_external_count >> BITS_PTR);

                }
                void set_external_count(int exCount){
                    intptr_t tmpPtr = (ptr_and_external_count << BITS_COUNTER) >> BITS_COUNTER;
                    ptr_and_external_count = ((uintptr_t)tmpPtr | ((unsigned long long)exCount << BITS_PTR));
                }
                node_type* get_ptr(){
                    intptr_t tmpPtr = (ptr_and_external_count << BITS_COUNTER) >> BITS_COUNTER;
                    return (node_type *)tmpPtr;
                }
                void set_ptr(node_type * ptr){
                    uintptr_t tmpExtCount = ptr_and_external_count >> BITS_PTR;
                    ptr_and_external_count = (((uintptr_t)ptr & MASK) | (tmpExtCount << BITS_PTR));
                }
            };
#else
#define BITS_INTERNAL_COUNT 30
            struct counted_node_ptr
            {
                ...
                void set_external_count(int exCount){
                    external_count = exCount;
                }
                node_type* get_ptr(){
                    return ptr;
                }
                void set_ptr(node_type * _ptr){
                    ptr = _ptr;
                }
            };
#endif
            ...
            */
            struct node_counter
            {
                size_t internal_count : BITS_INTERNAL_COUNT;
                size_t external_counters : 2;
            };
        ...
        protected:
            ...
            {
                node_type * const current_tail_ptr = old_tail.get_ptr();
                while (!m_Tail.compare_exchange_weak(old_tail, new_tail)
                       && old_tail.get_ptr() == current_tail_ptr);
            ...
            bool enqueue_value_ptr(scoped_value_ptr& value_ptr)
            {
                ...
                while (true)
                {
                    ...
                    node_type * n = old_tail.get_ptr();
                    if (n->m_value.compare_exchange_strong(old_value, value_ptr.get()))
                    {

                        counted_node_ptr old_next = { 0 };
                        if (!old_tail.get_ptr()->m_next.compare_exchange_strong(old_next, new_next))
                        {
                            free_node(new_next.get_ptr());
                            new_next = old_next;
                        }
        ...
        public:
            ...
            WilliamsQueue()
            {
                ...
                dummy_node.set_ptr(alloc_node());
                dummy_node.set_external_count(1);
            ...
            {
                clear();
                assert(m_Head.load().get_ptr() == m_Tail.load().get_ptr());
                free_node(m_Head.load().get_ptr());
            }
            ...
            */
            ...
            bool dequeue_with(Func f)
            {
                ...
                while (true)
                {
                    ...
                    node_type * const p = old_head.get_ptr();
                    if (p == m_Tail.load().get_ptr())
                    {
                        release_ref(p);
                        return false;
                    }
            ...
            */
            ...
            bool empty() const
            {
                return m_Head.load().get_ptr() == m_Tail.load().get_ptr();
            }
